...
static constexpr int INVALID_INDEX_removeme = -1;

RepairPanelModel::RepairPanelModel(QObject* parent)
...
QVariantMap RepairPanelModel::get(int index)
{
    QVariantMap result;
...
QVariant RepairPanelModel::data(const QModelIndex& index, int role) const
{
...
int RepairPanelModel::rowCount(const QModelIndex&) const
{
    return m_mixerChannelList.count();
}

QHash<int, QByteArray> RepairPanelModel::roleNames() const
{
...
void RepairPanelModel::loadItems()
{
    TRACEFUNC;
...
void RepairPanelModel::onTrackAdded(const TrackId& trackId)
{
    TRACEFUNC;
...
void RepairPanelModel::addItem(MixerChannelItem* item, int index)
{
    TRACEFUNC;
...
void RepairPanelModel::removeItem(const TrackId trackId)
{
    TRACEFUNC;

    int index = indexOf(trackId);
    if (index == INVALID_INDEX_removeme) {
        return;
    }
...
void RepairPanelModel::updateItemsPanelsOrder()
{
    TRACEFUNC;
...
void RepairPanelModel::setupConnections()
{
...
int RepairPanelModel::resolveInsertIndex(const engraving::InstrumentTrackId& newInstrumentTrackId) const
{
...
int RepairPanelModel::indexOf(const TrackId trackId) const
{
    for (int i = 0; i < m_mixerChannelList.size(); ++i) {
...
MixerChannelItem* RepairPanelModel::buildInstrumentChannelItem(const TrackId trackId,
...
MixerChannelItem* RepairPanelModel::buildAuxChannelItem(aux_channel_idx_t index, const TrackId trackId)
{
...
MixerChannelItem* RepairPanelModel::buildMasterChannelItem()
{
...
int RepairPanelModel::masterChannelIndex() const
{
    return m_mixerChannelList.size() - 1;
}

MixerChannelItem* RepairPanelModel::findChannelItem(const TrackId& trackId) const
{
    for (MixerChannelItem* item : m_mixerChannelList) {
...
void RepairPanelModel::loadOutputParams(MixerChannelItem* item, AudioOutputParams&& params)
{
    IF_ASSERT_FAILED(item) {
        return;
    }
...
void RepairPanelModel::updateOutputResourceItemCount()
{
    size_t maxFxCount = 0;
...
INotationProjectPtr RepairPanelModel::currentProject() const
{
    return context()->currentProject();
}

IProjectAudioSettingsPtr RepairPanelModel::audioSettings() const
{
    return currentProject() ? currentProject()->audioSettings() : nullptr;
}

INotationPlaybackPtr RepairPanelModel::notationPlayback() const
{
    return currentProject() ? currentProject()->masterNotation()->playback() : nullptr;
}

INotationPartsPtr RepairPanelModel::masterNotationParts() const
{
    return currentProject() ? currentProject()->masterNotation()->parts() : nullptr;
}

muse::ui::NavigationSection* RepairPanelModel::navigationSection() const
{
    return m_navigationSection;
}

void RepairPanelModel::setNavigationSection(muse::ui::NavigationSection* navigationSection)
{
...
int RepairPanelModel::navigationOrderStart() const
{
    return m_navigationOrderStart;
}

void RepairPanelModel::setNavigationOrderStart(int navigationOrderStart)
{
